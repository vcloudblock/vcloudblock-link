 ...
 * @readonly
 */
const DEFAULT_USER_DATA_PATH = path.join(__dirname, '../../.vcloudblockData');
 ...
 * @readonly
 */
const SERVER_NAME = 'vcloudblock-link-server';
 ...
 */
class VCloudBlockLink extends Emitter{
    /**
     * Construct a VCloudBlock link server object.
    ...
    listen (port, host) {
        ...
        this._httpServer.on('error', e => {
            this.isSameServer('127.0.0.1', this._port).then(isSame => {
                if (isSame) {
                    console.log(`Port is already used by other vcloudblock-link server, will try reopening after ${REOPEN_INTERVAL} ms`); // eslint-disable-line max-len
                    setTimeout(() => {
        ...
        this._httpServer.listen(this._port, '0.0.0.0', () => {
            this.emit('ready');
            console.info(clc.green(`VCloudBlock link server start successfully, socket listen on: http://${this._host}:${this._port}`));
        });
    }
}

module.exports = VCloudBlockLink;
